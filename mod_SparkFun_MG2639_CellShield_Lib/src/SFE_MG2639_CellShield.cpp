...
void MG2639_Cell::powerPinOn()
{
	digitalWrite(CELL_ON_OFF, HIGH);		// Writing high will start the pulse
}


void MG2639_Cell::powerPinOff()
{
	digitalWrite(CELL_ON_OFF, LOW);		// Writing low will end the pulse
}

uint8_t MG2639_Cell::powerOff()
{

	sendATCommand(POWER_OFF);

}

uint8_t MG2639_Cell::cellReset()
{

	sendATCommand(CELL_RESET);
...
int8_t MG2639_Cell::getATTOne(char * attOneRet)
{
	int iRetVal;
	
	sendATCommand(ATT_ONE); // Send "AT+CIMI"
	...
	if (iRetVal > 0)
	{
		...
		getSubstringBetween(attOneRet, (const char *) rxBuffer, '\n', '\r');
	}
	
	return iRetVal;
}

int8_t MG2639_Cell::getATTTwo(char * attTwoRet)
{
	int iRetVal;
	
	sendATCommand(ATT_TWO); // Send "AT+CIMI"
	...
	{
		// Get the substring between the first \n and second \r:
		getSubstringBetween(attTwoRet, (const char *) rxBuffer, '\n', '\r');
	}
	
	return iRetVal;
}

int8_t MG2639_Cell::getATTThree(char * attThreeRet)
{
	int iRetVal;
	
	sendATCommand(ATT_THREE); // Send "AT+CIMI"
	...
	{
		// Get the substring between the first \n and second \r:
		getSubstringBetween(attThreeRet, (const char *) rxBuffer, '\n', '\r');
	}
